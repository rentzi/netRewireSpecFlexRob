...
# communitiesDict: a dictionary with the indices in A of each community, i.e. communitiesDict[1] = the indices for the
# first cluster
...
# GETMODULARITYVALUESPTAUS gets the modularity values for all the combinations of pRandRewires and taus and store them in Q
# INPUT:
...
# OUTPUT:
# Q: a matrix getting all the modularity values, ex Q[i,j] = modularity for pRandRewires[i] and taus[j]


def getModularityValuesPTaus(vertices, edges, rewirings, pRandRewires, taus, binaryFlag='True'):

    Q = np.zeros((len(pRandRewires), len(taus)))
    ...
    for yy, p in enumerate(pRandRewires):
        for xx, t in enumerate(taus):
            if binaryFlag == 'True':
                ...
                Arand = swn.generateWeightRandSymAdj(vertices, edges, weightDistribution='normal')
            elif binaryFlag == 'lognormal':
                Arand = swn.generateWeightRandSymAdj(vertices, edges, 'lognormal')
            ...
            # print(Qlist)
            Q[yy, xx] = np.sum(Qlist)

    return Q

# GETMODULARITYVALUESPTAUSREPETITIONS  does the same thing as GETMODULARITYVALUESPTAUS but for many repetitions
# Input:
# same as GETMODULARITYVALUESPTAUS except for repetitions
# repetitions: number of repetitions you calculate Q for the same parameters
# Output:
# QAll[:,:,i] = the values of all the pairs (pRandRewires,taus) for the i-th iteration


def getModularityValuesPTausRepetitions(vertices, edges, rewirings, pRandRewires, taus, binaryFlag, reps):

    QAll = np.zeros((len(pRandRewires), len(taus), reps))
    for rep in np.arange(reps):
        print(rep + 1)
        QAll[:, :, rep] = getModularityValuesPTaus(vertices, edges, rewirings, pRandRewires, taus, binaryFlag)

    return QAll


# PLOTMODULARITYVALUESPTAUS plots the matrix Q that contains modularity values for different p(random) and tau
# INPUT:
# Q: the modularity matrix
# pRandRewires: the list with the p(random) values
# taus: the list with the tau values
# cmap: default to viridis


def plotModularityValuesPTaus(Q, pRandRewires, taus, cmap='viridis', filePath='False', clim=[0, 0.8]):

    plt.imshow(Q, cmap=cmap)
    plt.xlabel('tau')
    plt.ylabel('P(random)')
    plt.yticks(np.arange(len(pRandRewires)), pRandRewires), plt.xticks(np.arange(len(taus)), taus)
